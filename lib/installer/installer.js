...
class Installer {
  ...
  * install() {
    ...
    let nodeDir = path.join(nodeModulesDir, 'node');
...
function unzipFile(zipFile, targetDir) {
  if (typeof zipFile === 'string') {
    if (!zipFile || !fs.existsSync(zipFile) || !fs.statSync(zipFile).isFile())
      throw new Error(`[unzipFile] zipFile should be one existed filename!`)
