...
String.prototype._trimEOL = function() {
  return this.replace('\r\n', '').replace('\n', '');
}
...
describe('test/install_node_win32.test.js', function() {
  ...
  it('should install node', function* () {
    ...
    assert(execSync(`${nodeBinPath} -v`).toString()._trimEOL() === 'v6.2.2');
    assert(execSync(`\"${npmBinPath}\" -v`, { env: getEnv(nodeBinPath) }).toString()._trimEOL() === '3.9.5');
  });

  it('should install noderc', function* () {
    cwd = path.join(fixtures, 'install-node');
    yield coffee
      .fork(nodeinstall, [ '--install-noderc', '6.9.2-rc.1', '--china' ], { cwd })
      .debug()
      .expect('code', 0)
      .end();
    ...
    assert(fs.existsSync(nodeDir));
    assert(execSync(`${nodeBinPath} -v`).toString()._trimEOL() === 'v6.9.2-rc.1');
  });
  ...
  it('should install node nightly', function* () {
    ...
    assert(/v\d+.\d+.\d+-nightly[a-z0-9]{18}/.test(
      execSync(`${nodeBinPath} -v`).toString()._trimEOL()
    ));
  });
